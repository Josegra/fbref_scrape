...
output_dir = './data'
output_filename = 'shooting_outside_top5.csv'
...
if dfs:
    shooting_df = pd.concat(dfs, ignore_index=True)
    df_cleaned = shooting_df[~shooting_df['Player'].str.contains('Player|Squad Total', na=False)]
    
    # --- REORDENAR COLUMNAS (OPCIONAL PERO RECOMENDADO) ---
    # Colocar 'League' y 'Squad' al principio para mayor claridad.
    cols = ['League', 'Squad', 'Player'] + [col for col in df_cleaned.columns if col not in ['League', 'Squad', 'Player']]
