...
leagues_urls = {
    "Big5": ["https://fbref.com/en/comps/Big5/gca/players/Big-5-European-Leagues-Stats"]
}

# --- CONFIGURACIÓN PARA EXPORTAR ---
output_dir = './data'
output_filename = 'gca_big5.csv'
...
for league, urls in leagues_urls.items():
    for url in urls:
        try:
            ...
            # CAMBIO: Seleccionar la tabla de "Goal and Shot Creation" (índice 6).
            df = df_list[0]
            ...
            # --- LÓGICA DE RENOMBRAMIENTO DE COLUMNAS PARA GCA/SCA ---
            if isinstance(df.columns, pd.MultiIndex):
                new_cols = []
                for level0, level1 in df.columns:
                    # Columnas sin encabezado superior (Player, Nation, etc.)
                    if 'Unnamed' in level0:
                        new_cols.append(level1)
                    # Columnas bajo los encabezados "SCA Types" o "GCA Types"
                    elif 'Types' in level0:
                        # Determinamos el sufijo basado en el encabezado superior
                        suffix = 'SCA' if 'SCA' in level0 else 'GCA'
                        new_cols.append(f"{level1}_{suffix}")
                    # Columnas principales (SCA, GCA, SCA90, GCA90)
                    else:
                        new_cols.append(level1)
                df.columns = new_cols
...
if dfs:
    ...
    gca_sca_df = pd.concat(dfs, ignore_index=True)
